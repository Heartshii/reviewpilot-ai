...
import {
  extractSharedSecret,
  normalizeReservationWebhookPayload,
  providerFromPath,
} from "@/lib/integration-webhooks";
import { isReservationProvider } from "@/lib/integrations";
...
export async function POST(
  ...
  if (!provider || !isReservationProvider(provider)) {
    return NextResponse.json(
      { error: "Unknown reservation provider." },
      { status: 404 }
    );
  }
  ...
  try {
    const normalized = normalizeReservationWebhookPayload(provider, payload);
    const result = await convex.mutation(api.integrations.ingestReservationWebhook, {
      provider,
      token,
      sharedSecret,
      ...normalized,
